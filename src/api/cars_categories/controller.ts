...
const Controller = (db: ExtendedProtocol) => {
  ...
  const create = async (req: Request) => {
    ...
    try {
      const row = await db.cars_categories.create(cars_category);
      if (!row) {
  ...
  const updateById = async (req: Request) => {
    ...
    try {
      ...
      const rows = await db.cars_categories.update(updatedData, _id);
      if (!rows) {
        throw new ErrorResponse(400, 'Please send the correct info');
      }
      return rows;
  ...
  const deleteById = async (req: Request) => {
    ...
    try {
      ...
      const row = await db.cars_categories.destroy(_id);
      if (!row) {
