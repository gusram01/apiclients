...
const Controller = (db: ExtendedProtocol) => {
  const some = async (req: Request) => {
    try {
      const rows = await db.roles.find({ ...req.query, active: true });
      if (!rows) {
  ...
  const findById = async (req: Request) => {
    ...
    try {
      const rows = await db.roles.find({ active: true, _id });
  ...
  const create = async (req: Request) => {
    ...
    try {
      const row = await db.roles.create(newData);
      if (!row) {
        throw new ErrorResponse(400, 'Please send the correct info');
      }
  ...
  const updateById = async (req: Request) => {
    ...
    try {
      const row = await db.roles.update(updatedData, id);
      if (!row) {
        throw new ErrorResponse(400, 'Please send the correct info');
      }
      return row;
  ...
  const deleteById = async (req: Request) => {
    const _id = req.params.id;
    try {
      const data = await db.roles.find({ active: true, _id });
      if (!data) {
      ...
      const rows = await db.roles.update(deleteUser, _id);
      if (!rows) {
